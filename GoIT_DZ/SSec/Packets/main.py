...
for i in range(len(parsed_config["admins"])):
    encrypted_admin_login = cipher.encrypt(
        parsed_config["admins"][i]["login"].encode())
    parsed_config["admins"][i]["login"] = encrypted_admin_login.decode()

    encrypted_admin_password = cipher.encrypt(
        parsed_config["admins"][i]["password"].encode())
...
encrypted_session_key = cipher.decrypt(
    parsed_protected_config["session_key"].encode())
parsed_protected_config["session_key"] = encrypted_session_key.decode()

for i in range(len(parsed_protected_config["admins"])):
    encrypted_admin_login = cipher.decrypt(
        parsed_protected_config["admins"][i]["login"].encode())
    parsed_protected_config["admins"][i]["login"] = encrypted_admin_login.decode(
    )

    encrypted_admin_password = cipher.decrypt(
        parsed_protected_config["admins"][i]["password"].encode())
