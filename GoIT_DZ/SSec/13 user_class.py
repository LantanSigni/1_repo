...
class AccessObject:
    '''Клас об'єкту, до якого надається доступ, зі своїм вмістом, назвою цього об'єкта та вказуванням поточного власника.'''
    content = ''
    name = ''
    owner = User()
    ...
    def change_owner(self, old_owner_password, new_owner):
        ...
        if isinstance(new_owner, AuthenticatedUser) and isinstance(self.owner, AuthenticatedUser) and old_owner_password == self.owner.password:
...
if __name__ == '__main__':  # перевірка коду.
    # alice = User()
